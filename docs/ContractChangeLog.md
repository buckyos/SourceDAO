...
1. `upgradeToAndCall(...)` 必须绑定治理批准的 `calldataHash`
2. 普通 `upgrade proposal` 的正常通过、拒绝、过期取消
3. 已配置系统下的 `Dao` / `Committee` 升级

但还有一个更贴近真实治理使用的时序需要单独固定：

1. 一个旧的 `upgrade proposal` 还处于悬挂状态
2. 中途发生委员会换届
3. 换届后的新委员会想为同一个 proxy 再发起一个新的升级提案
4. 最终再执行 `upgradeToAndCall(...)`

这里最容易出错的点有两个：

1. 旧提案是否仍然绑定旧委员会快照
2. 同一个 proxy 的旧升级队列，是否会把后续新提案的边界搞乱

### 具体改动

在 `test/upgrade.ts` 新增一条并发治理回归，固定以下语义：

1. 旧委员会先为 `Dao` 发起一个普通 `upgrade proposal`，但只获得一票支持，保持悬挂
2. 旧委员会再通过普通 `setCommittees` 提案完成换届
3. 换届后：
   - 新委员会中的新成员不能去补投那个旧升级提案
   - 已被移出的旧委员会成员仍然可以按旧快照继续参与那个旧升级提案
4. 在旧升级提案仍然挂在 `contractUpgradeProposals[proxy]` 时，新委员会不能再为同一个 proxy 发起第二个升级提案
5. 等旧升级提案过期后，由新委员会成员执行 `cancelContractUpgrade(...)` 清理旧队列
6. 队列清理后，新委员会可以重新发起新的升级提案，并成功执行 `upgradeToAndCall(...)`

### 当前结论

这条测试把升级治理与委员会换届之间的边界固定得更明确：

1. 旧升级提案不会因为换届而自动切到新委员会
2. 旧升级提案仍然受其创建时的委员会快照约束
3. 同一个 proxy 在旧升级提案未清理前，不能并发排第二个升级提案
4. 只有旧提案过期并被清理后，新委员会才能重新为该 proxy 发起新的升级提案
5. 新升级提案在换届后的新委员会快照下独立生效，最终可以正常执行 `upgradeToAndCall(...)`

### 验证方式

执行：

1. `npx hardhat test test-hh3/upgrade.ts`
2. `npm test`

结果：

- `upgrade.ts` focused suite 通过
- 全量测试通过，当前总数：`259 passing`
...
在线投票脚本虽然已经整理到 `tools/vote.ts`，但它仍然要求：
...
本次新增最小版离线签名方案，不改 `Committee` 合约，只改工具层：

1. 新增 `tools/vote_offline.ts`
2. 支持三步流程：
   - `prepare`
   - `sign`
   - `broadcast`
3. 新增独立文档 `docs/VoteOffline.md`
4. `README.md` 和 `docs/VoteTool.md` 同步补充离线签名入口说明

### 方案边界

这一版的核心特征是：

1. 在线机器负责读取 proposal、拉取后台参数、构造原始交易
2. 离线机器只负责签名原始交易
3. 在线机器只负责广播已签名交易

它解决的是“私钥不再必须放在联网机器”，但它还不是：

1. `vote by signature`
2. EIP-712 typed data 投票
3. 合约级 `supportBySig / rejectBySig`
...
2. 能快速落地
3. 能先把私钥从联网机器移走
4. 为后续真正的 `vote by signature` 方案预留演进空间

## Vote Tool JSON 配置文件支持

### 背景

前一版工具主要通过环境变量传配置，虽然已经能工作，但在日常运维里有两个明显问题：

1. 常用配置需要重复输入
2. `vote.ts` 和 `vote_offline.ts` 之间的配置不好复用

### 本次补充

本次补了共享 JSON 配置文件支持：

1. 新增公共读取逻辑到 `tools/vote_common.ts`
2. `vote.ts` 和 `vote_offline.ts` 现在都支持读取：
   - `vote.config.json`
   - `tools/vote.config.json`
   - 或 `SOURCE_DAO_CONFIG` 指向的 JSON 文件
3. 新增示例文件 `tools/vote.config.example.json`
4. 文档同步补充配置文件说明
...
到目前为止，`vote.ts` / `vote_offline.ts` 已经有了多轮功能迭代，但还没有独立自动化测试覆盖。

对工具层来说，最需要先固定的是离线签名的安全边界和文件流转语义。

### 本次补充

新增 `test/vote_tool.ts`，优先覆盖离线签名模式的 4 条关键路径：

1. `sign` 模式能通过 JSON 配置文件和相对路径生成已签名 bundle
2. `sign` 模式会拒绝与 bundle voter 不匹配的私钥
3. `broadcast` 模式能广播已签名 bundle，并回写 tx hash
4. `broadcast` 模式会拒绝目标地址与 bundle 中 Committee 地址不一致的交易

同时新增 `test-hh3/vote_tool.ts` 入口，让这批工具测试进入当前 Hardhat 3 测试套件。

### 当前结论

这批测试不是在验证合约逻辑，而是在固定工具层自己的行为边界：
...
## 2026-03-19 full proposal 批量结算边界补测

### 范围

- 测试：`test/committee.ts`

### 背景

针对 full proposal，目前还有两类需要明确固定的行为边界：

1. 当前实现按“结算当下余额”计票，且允许分批 `endFullPropose(...)`，因此同一批 token 在不同 batch 之间转移后，可能被重复计权
2. 即使保留这条已知设计问题，分批结算本身在更大 voter 集合上仍应保持计数稳定，不因为重复地址或批次切分而额外漂移

### 具体改动

#### 1. 固定跨 batch 转移会双算的已知问题

新增一条表征测试，覆盖：

1. 两个地址都先对同一个 full proposal 投票
2. 先只结算第一个地址
3. 第一个地址在被结算后，把自己的票权通过 `dev2normal + transfer` 转给第二个地址
4. 再单独结算第二个地址

当前结果是：

1. 第一个地址的旧票权已经记入 `agree`
2. 第二个地址会按收到转移后更高的当前余额再次记票
3. 最终 `agree` 会达到 `8_000`
4. 同一个 full proposal 因此被提前接受

这条测试的目的不是认可该语义，而是把它作为当前已知设计问题固定下来，避免后续误判为“已经天然防住”。

#### 2. 增加多 voter / 多 batch settle 的规模化回归

新增一条更贴近实际治理规模的 full proposal 测试，覆盖：

1. 13 个 voter 参与同一个 full proposal
2. 通过 `dev2normal + transfer` 先把票权分发到多个额外地址
3. 结算拆成 3 个 batch
4. 中途包含重复地址输入
...
2. `reject = 2_250`
3. `settled = 13`
4. `totalReleasedToken = 7_000`
5. proposal 最终被正确接受

这条测试的重点，是固定当前实现下“批量 settle 过程本身”的数值一致性，而不是改变 full proposal 的权重模型。
