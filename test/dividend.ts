...
describe("dividend", function () {
    ...
    it("reverts native dividend payouts atomically when the recipient rejects the transfer", async function () {
        ...
        await expect(
            receiver.withdrawDividends(await dividend.getAddress(), [1n], [ethers.ZeroAddress])
        ).to.be.revertedWith("native transfer failed");

        expect(await dividend.getDepositTokenBalance(ethers.ZeroAddress)).to.equal(60n);
        expect(await receiver.receiveCount()).to.equal(0n);
        expect(await receiver.totalReceived()).to.equal(0n);

        await (await receiver.setRejectReceive(false)).wait();
        await (await receiver.withdrawDividends(await dividend.getAddress(), [1n], [ethers.ZeroAddress])).wait();
