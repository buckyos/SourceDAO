...
8. 只读状态工具见`tools/dao_status.ts`、`tools/committee_status.ts`和`tools/proposal_status.ts`
...
8. Read-only status helpers are available in `tools/dao_status.ts`, `tools/committee_status.ts`, and `tools/proposal_status.ts`
